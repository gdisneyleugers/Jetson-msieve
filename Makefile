...
CC = nvcc
CCC = nvcc
WARN_FLAGS = -gencode arch=compute_53,code=sm_53 
OPT_FLAGS = -O3 \
...
all: $(COMMON_OBJS) $(QS_OBJS) $(NFS_OBJS) $(GPU_OBJS)
	...
	ranlib libmsieve.a
	$(CCC) $(CFLAGS) demo.c -o msieve $(LDFLAGS) \
			libmsieve.a $(LIBS)
...
stage1_core_sm20.ptx: $(NFS_GPU_HDR)
	$(NVCC) --preprocess -gencode arch=compute_20,code=sm_20 -ptx -o $@ $<

stage1_core_sm30.ptx: $(NFS_GPU_HDR)
	$(NVCC) --preprocess -gencode arch=compute_30,code=sm_30 -ptx -o $@ $<

stage1_core_sm35.ptx: $(NFS_GPU_HDR)
	$(NVCC) --preprocess -gencode arch=compute_35,code=sm_35 -ptx -o $@ $<

stage1_core_sm53.ptx: $(NFS_GPU_HDR)
	$(NVCC) --preprocess -gencode arch=compute_53,code=sm_53 -ptx -o $@ $<

cub/built:
	cd cub && make sm=200,300,350,530 && cd ..
